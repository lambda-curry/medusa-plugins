...
export const createMissingProductReviewStatsStepId =
	"create-missing-product-review-stats";

export const createMissingProductReviewStatsStep = createStep<
	string[],
	ProductReviewStats[],
	string[]
>(
	...
	async (productIds: string[], { container }) => {
		...
		const missingStats = productIds.filter(
			(productId) => !stats.some((stat) => stat.product_id === productId),
		);
