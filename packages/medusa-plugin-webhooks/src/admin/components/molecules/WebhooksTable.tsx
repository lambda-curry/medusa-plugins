...
export const columnsDef = ({
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <>
          <StatusIndicator title={finalStatus.title} variant={finalStatus.variant} />
          {row.original.active}
        </>
      );
    },
...
export const WebhooksTable: React.FC<WebhooksTableProps> = ({
  ...
  const { data: subscriptionsData, refetch: refetchWebhooks } = useAdminWebhooks({
    ...
    <>
      <DataTable
        ...
        onPaginationChange={(updater) => {
          const update = typeof updater === 'function' ? updater(paginationState) : updater;
